...
var displayMap = function displayMap(locations) {
  ...
  locations.forEach(function (loc) {
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, ": ").concat(loc.description, "</p>")).addTo(map); // Extend map bounds to incude current location
...
/* eslint-disable */
var stripe = Stripe('pk_test_KxVJfyTeTKR36Ah9t9Vgql4e00PQLfQfpQ');
...
if (logoutBtn) logoutBtn.addEventListener('click', _login.logout); // Update name, email, photo

if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault(); // For photo
